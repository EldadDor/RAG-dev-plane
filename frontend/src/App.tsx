    ...
    <section className="workspace" aria-label="Chat workspace">
      ...
      <section className="chat-pane" aria-label="Chat" aria-busy={Boolean(streamController.current)}>
        ...
        <div className={activeSession ? 'conversation-view' : 'empty-state'} aria-live="polite">
          ...
          {relatedImages.length > 0 && <section className="related-images" aria-labelledby="related-images-heading"><h2 id="related-images-heading">Related source images</h2><div>{relatedImages.map(({ source, asset }) => <figure key={asset.assetId}><a href={asset.contentUrl ?? undefined} target="_blank" rel="noreferrer"><img src={asset.contentUrl ?? undefined} alt={asset.altText ?? asset.caption ?? `Image from ${source.title ?? source.sourcePath}`} loading="lazy" /></a><figcaption>{asset.caption ?? asset.altText ?? `Image from ${source.title ?? source.sourcePath}`}</figcaption></figure>)}</div></section>}
        </div>
      ...
      <aside className="sessions-pane" aria-label="Recent chats">
        ...
        {activePane === 'sources' && <div aria-live="polite">{grounded === false && <p className="muted">This answer was not grounded in retrieved sources.</p>}{sources.length === 0 && grounded !== false && <p className="muted">No sources are available yet.</p>}{sources.length > 0 && <ol className="source-list">{sources.map((source) => <li key={source.chunkId}><strong>{source.title ?? source.sourcePath}</strong><span>{source.sourcePath}{source.section ? ` · ${source.section}` : ''}{source.page !== null ? ` · Page ${source.page}` : ''}</span>{source.snippet && <p>{source.snippet}</p>}{source.assets.filter((asset) => asset.contentUrl).map((asset) => <figure key={asset.assetId}><a href={asset.contentUrl ?? undefined} target="_blank" rel="noreferrer"><img src={asset.contentUrl ?? undefined} alt={asset.altText ?? asset.caption ?? `Image from ${source.title ?? source.sourcePath}`} loading="lazy" /></a>{(asset.caption || asset.altText) && <figcaption>{asset.caption ?? asset.altText}</figcaption>}</figure>)}</li>)}</ol>}</div>}
