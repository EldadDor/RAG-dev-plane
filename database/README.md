...
For an existing database, these migrations adopt the current tables and workspace values without dropping data. Migration `003_chunking_profiles.sql` marks every existing source and chunk as the `default` profile; it does not re-embed or delete indexed data. The local seed is idempotent. Production identity memberships must be provisioned separately by an approved administrative process.
